...
from model import resnet18_mask
...
def arg_pare():
	...
	arg.add_argument('--modeldir', help=' the model viz dir ', default='ResNet18_mask')
...
def main():
	print('\n loading the dataset ... \n')
	print('\n done \n')
	model = resnet18_mask(pretrained=True, num_classes=args.num_classes).cuda()
...
def train(trainloader, model, criterion, optimizer, epoch):
	...
	for i, (input, target,true_mask) in enumerate(trainloader):
		...
		input, target, true_mask = input.cuda(), target.cuda(),true_mask.cuda()
		out1, _, _ ,mask= model(input)
		prec1, prec2 = accuracy(out1, target.long(), dir=None,path=None, topk=(1,2))
		loss = criterion(out1, target.long())+F.binary_cross_entropy_with_logits(mask,true_mask.unsqueeze(1))
...
def evaluate(valloader, model, criterion,is_last,args,epoch):
	...
	with torch.no_grad():
		...
		for i, (path,input, target) in enumerate(valloader):
			input, target = input.cuda(), target.cuda()
			output1, _, _, mask = model(input)
			loss = criterion(output1, target.long())
			if epoch==args.epochs-1:
				plot_mask(input,mask)
			path=path if is_last else None
			prec1, prec2 = accuracy(output1, target.long() ,args.modeldir ,path=path, topk=(1, 2))
			losses.update(loss.item(), input.size(0))
			top1.update(prec1[0], input.size(0))
			top2.update(prec2[0], input.size(0))
			batch_time.update(time.time() - end)
...
def plot_mask(input,masks):
	...
	for i in range(input.shape[0]):
		...
		image= cv2.addWeighted(image,0.6,mask,0.4,20)
